...
class User(db.Model):
    ...
    password_hash = db.Column(db.String(128), nullable=False)                           # 비밀번호는 해시값으로 저장
    ...
    def set_password(self, password):
        self.password_hash = generate_password_hash(password)                           # 비밀번호를 해싱하여 저장

    # 해시된 비밀번호와 입력된 비밀번호를 비교
    def check_password(self, password):
        return check_password_hash(self.password_hash, password)                         # 해시된 비밀번호와 입력된 비밀번호를 비교
