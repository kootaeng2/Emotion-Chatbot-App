...
│       └── sync-to-hub.yml
│
├── data/                        # AI 모델 학습에 사용된 원본 및 레이블링 데이터
│   ├── training-label.json
│   ├── training-origin.xlsx
...
│   ├── auth.py                  # 회원가입, 로그인/아웃 등 인증 관련 로직
│   ├── emotion_engine.py        # 감정 분석 모델 로딩 및 예측
│   ├── main.py                  # 메인 페이지, 일기 저장/조회 등 핵심 기능
│   ├── models.py                # 데이터베이스 테이블 모델 (사용자, 일기)
│   └── recommender.py           # 감정 기반 콘텐츠 추천 로직
│
